...
function normalizeCreate(params) {
    ...
    if (!birthDatePattern.test(birthDate)) {
        throw new Error("`birthDate` should be a valid date in the format YYYY-MM-DD.");
    }
    ...
    if (document) {
        ...
        if (document.length !== 11 && document.length !== 14) {
            throw new Error("`document` should have 11 digits (for CPF) or 14 digits (for CNPJ).");
        }
    }
}
